...
var SockJS = function(url, dep_protocols_whitelist, options) {
    ...
    that._options = {
        ...
        info: undefined, 
        rtt: undefined, 
        heartbeatTimeout: 0 // Heartbeat timeout in ms (0 means disabled)
    };
...
SockJS.prototype._dispatchOpen = function() {
    ...
    if (that.readyState === SockJS.CONNECTING) {
        // If heartbeat timeout is enabled
        if (that._options.heartbeatTimeout > 0) {
            that._setHeartbeatTimeout();
        }
...
SockJS.prototype._setHeartbeatTimeout = function(data) {
    var that = this;
    clearTimeout(that.heartbeatTimeoutTimer);
    
    // Set the timeout that will call onclose from as soon as the timeout has been reached
...
SockJS.prototype._didClose = function(code, reason, force) {
    ...
    // Ready state = CLOSED when timeout occurred on the server
    if (that.readyState === SockJS.CLOSED) {
        return;
    }
...
SockJS.prototype._didMessage = function(data) {
    ...
    // If heartbeatTimeout is enabled
    if (that._options.heartbeatTimeout > 0) {
        // We received data, clear the heartbeat timeout
        that._setHeartbeatTimeout();
    }
