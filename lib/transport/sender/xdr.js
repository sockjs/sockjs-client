...
class XDRObject extends EventEmitter {
  ...
  _cleanup(abort) {
    ...
    this.xdr.removeEventListeners('load', this._xdrload);
    this.xdr.removeEventListeners('progress', this._xdrprogress);
    this.xdr.removeEventListeners('error', this._xdrerror);
    this.xdr.removeEventListeners('timeout', this._xdrtimeout);
    if (abort) {
      try {
        this.xdr.abort();
      } catch {
